...
Token.prototype.getRedToken = function () {
    return this.tokens[this.redToken];
}

Token.prototype.getYellowToken = function () {
    return this.tokens[this.yellowToken];
}
...
Checker.prototype.review = function (coordinates, coordinateOfLastTokenPlaced) {
    ...
    for (let i = 0; i < coordinates.length; i++) {
        ...
        while (isWithinTheRange(displacedCoordinate) && this.cells[coordinateOfLastTokenPlaced.getAxisY()][coordinateOfLastTokenPlaced.getAxisX() === this.cells[displacedCoordinate.getAxisY()][displacedCoordinate.getAxisX()]]) {
            count++;
    ...
    function isWithinTheRange(coordinate) {
        ...
        const RANGE_Y = 5;
        const MINOR_RANGE = 0;
        return MINOR_RANGE <= coordinate.getAxisX() &&
...
PlayerView.prototype.getValidColumn = function (player) {
    let error;
    let columnNumber;
    do {
        columnNumber = console.writeln(`Selecciona una columna para insertar FICHA ${player.getToken() === Toke.RED ? "ROJA" : "AMARILLA"}: `);
...
BoardView.prototype.show = function () {
    ...
    for (let i = 0; i < Board.rows; i++) {
        ...
        for (let j = 0; j < Board.columns; j++) {
            console.writeln(`${this.board.getCells()[i][j]} |`);
        }
        console.writeln();
    }
    console.writeln("----------------------");
...
Board.prototype.isConnect4 = function (coordinateOfLastTokenPlaced) {
    ...
    directions.forEach(coordinates => {
        if (checker.review(coordinates, coordinateOfLastTokenPlaced)) {
            return true;
        }
    });
...
Board.prototype.isTie = function () {
    this.cells.forEach(row => {
        row.forEach(column => {
            if (column.includes(Board.EMPTY_CHARACTER)) {
                return false;
            }
        })
    });
...
Board.prototype.add = function (token, column) {
    ...
    for (let i = Board.ROWS - 1; !placedToken && i >= 0; i--) { // Cambiar por un do while
...
function GameView() {
    ...
    // console.writeln(`Properties: ${Object.getOwnPropertyNames(this.board)}`);
    // console.writeln(`Values: ${Object.values(this.board)}`);
...
GameView.prototype.play = function () {
    ...
    while (!winner && !this.board.isTie()) {
        let columnToInsertToken = getValidAndNonFullColumn(this);
    ...
    if (winner) {
        this.boardView.showWinningMessage(this.getTurn());
    ...
    function getValidAndNonFullColumn(object) {
        ...
        let column;
        do {
            column = object.playerView.getValidColumn(object.players[object.turn.getTurn()]);
