...
Checker.prototype.review = function (coordinates, coordinateOfLastTokenPlaced) {
  ...
  for (let i = 0; i < coordinates.length; i++) {
    ...
    while (
      ...
      this.cells[coordinateOfLastTokenPlaced.getAxisY()][
        coordinateOfLastTokenPlaced.getAxisX() === this.cells[displacedCoordinate.getAxisY()][displacedCoordinate.getAxisX()]
      ]
    ) {
      count++;
...
PlayerView.prototype.getValidColumn = function (player) {
  ...
  let columnNumber;
  do {
    columnNumber = console.writeln(
      `Selecciona una columna para insertar FICHA ${player.getToken() === Token.RED ? "ROJA" : "AMARILLA"}: `
    );
...
Board.prototype.getCoordinateOfLastAddedToken = function (token, column) {
  console.writeln(`Method getCoordinateOfLastAddedToken ${token} ${column}`);
...
GameView.prototype.play = function () {
  ...
  while (!winner && !this.board.isTie()) {
    ...
    console.writeln(`Value columnToInsertToken ${columnToInsertToken}`);
